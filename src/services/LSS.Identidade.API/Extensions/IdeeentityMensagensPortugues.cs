...
namespace LSS.Identidade.API.Extensions
{
    public class IdeeentityMensagensPortugues: IdentityErrorDescriber
    {
        ...
        public override IdentityError InvalidUserName(string userName) { return new IdentityError { Code = nameof(InvalidUserName), Description = $"O login '{userName} é invalido, pode coonter apenas letras ou dígitos." }; }
        ...
        public override IdentityError UserAlreadyInRole(string role) { return new IdentityError { Code = nameof(UserAlreadyInRole), Description = $"O usuário já possui permissão para a role '{role}'." }; }
        ...
        public override IdentityError PasswordRequiresNonAlphanumeric() { return new IdentityError { Code = nameof(PasswordRequiresNonAlphanumeric), Description = $"As senhas devem conter pelo menos um caracteres especial." }; }
        ...
        public override IdentityError PasswordRequiresLower() { return new IdentityError { Code = nameof(PasswordRequiresLower), Description = $"As senhas devem contere ao menos um caracter em caixa baixa (a - z)." }; }
        public override IdentityError PasswordRequiresUpper() { return new IdentityError { Code = nameof(PasswordRequiresUpper), Description = $"As senhas devem contere ao menos um caracter em caixa alta (A - Z)." }; }
